...
def siamese_track_id(state, im, mask_enable=False, refine_enable=False, device='cpu', debug=False):
    p = state['p']
    net = state['net']
    ...
    net.template(state['z'].to(device))
...
def siamese_get_mask(state, im, best_pscore_id, mask_enable=False, refine_enable=False, device='cpu', debug=False):
    p = state['p']
    net = state['net']
    avg_chans = state['avg_chans']
    window = state['window']
    ...
    if mask_enable:
        score, delta, mask = net.track_mask(state['x_crop'].to(device))
    else:
        score, delta = net.track(state['x_crop'].to(device))
    ...
    if mask_enable:
        ...
        if len(contours) != 0 and np.max(cnt_area) > 100:
            ...
            rbox_in_img = np.array([[pbox[0],pbox[1]],[pbox[0]+pbox[2],pbox[1]+pbox[3]]])
        else:  # empty mask
